...
isWebp();
const preloadHiddens = document.querySelectorAll('._preload-hidden');

window.addEventListener('load', () => {
  preloadHiddens.forEach((hiddenObject) => {
    hiddenObject.classList.remove('_preload-hidden');
  })
})
...
let scrollBlockBody = document.querySelector('body');
...
burgerBtn.addEventListener('click', () => {
  if (burgerBtn.classList.contains('_active')) {
    ...
    scrollBlockBody.classList.remove('_no-scroll');
    removeScrollMargin();
  } else {
    burgerBtn.classList.add('_active');
    mobileMenu.classList.add('_active');
    scrollBlockBody.classList.add('_no-scroll');
    addScrollMargin();
  }
})
...
//single element adaptive menu
// 900
const headerMiddleForm = document.querySelector('.header-middle__form');
const headerAdaptiveSearchInner = document.querySelector('.header__adaptive-search-inner');
const headerMiddleColumn = document.querySelector('.header-middle__column');
// 650
...
const headerTopRow = document.querySelector('.header-top__row');
const catalogLink = document.querySelector('.catalog-link');
const menuMiddleLinkProfile = document.querySelector('.menu-middle__link--profile');
const menuMiddleLinkOrders = document.querySelector('.menu-middle__link--orders');
const menuBottomList = document.querySelector('.menu-bottom__list');
const menuTopList = document.querySelector('.menu-top__list');
const headerBottomMenu = document.querySelector('.header-bottom__menu');

function maxWidth650() {
  if (window.matchMedia('(max-width: 650px)').matches) {
    mobileMenuInner.prepend(menuTop);
    menuTop.prepend(catalogLink);
    catalogLink.after(menuMiddleLinkProfile);
    menuMiddleLinkProfile.after(menuMiddleLinkOrders);
    menuTopList.before(menuBottomList);
    menuMiddleLinkOrders.after(choiceCity);
  } else {
    headerTopRow.append(menuTop);
    headerMiddleColumn.prepend(catalogLink);
    catalogLink.after(menuMiddleLinkProfile);
    menuMiddleLinkProfile.after(menuMiddleLinkOrders);
    headerBottomMenu.prepend(menuBottomList);
    headerTopRow.prepend(choiceCity);
  }
};
...
function maxWidth900() {
  if (window.matchMedia('(max-width: 900px)').matches) {
    headerAdaptiveSearchInner.prepend(headerMiddleForm);
  } else {
    headerMiddleColumn.prepend(headerMiddleForm);
  }
};
...
document.querySelectorAll('.dynamic-transfer').forEach((transferItem) => {
  ...
  const itemAttrArr = itemAttr.split(',');
...
const heroSlider = new Swiper('.hero-slider', {
  ...
  loop: true,
  autoplay: {
    delay: 3000, //по умолчанию 3000 мс
    stopOnLastSlide: false,
    disableOnInteraction: true,
  },
  effect: 'fade',
...
document.querySelectorAll('.custom-select').forEach((customSelect) => {
  ...
  for (let i = 0; i < customSelectItem.length; i++) {
    if (customSelectItem[i].hasAttribute('data-active')) {
...
moreBlock.forEach((block) => {
  const moreBody = block.querySelector('.more-block__body');
  const moreBtn = block.querySelector('.more-block__btn');

  let moveTransition = 1;
  let moreBodyHeight = 500;
  let moreBodyHeightReal = moreBody.clientHeight;

  moreBody.style.overflow = 'hidden';
  moreBody.style.transition = `height ${moveTransition}s`;
  ...
  moreBtn.addEventListener('click', function () {
    moreBody.classList.toggle('_more');
    if (moreBody.classList.contains('_more')) {
      moreBody.style.height = `${moreBodyHeightReal}px`;
      moreBtn.style.display = 'none';
      setTimeout(() => {
        moreBody.style.height = 'auto';
      }, 1000);
    }
  })

  function blockHeightInit () {
    if (window.matchMedia('(max-width: 600px)').matches) {
      moreBody.style.height = `${moreBodyHeight}px`;
      moreBtn.style.display = 'block';
      moreBody.classList.remove('_more');
    } else {
      moreBody.style.height = 'auto';
      moreBtn.style.display = 'none';
    }
...
// fixed header
const fixedHeaderBlock = document.querySelector('.header__middle');
const headerBlock = document.querySelector('.header');
const headerTop = document.querySelector('.header__top');

let fixedHeaderBlockTopDistance = fixedHeaderBlock.getBoundingClientRect().top;
let headerBlockDistance = headerBlock.getBoundingClientRect().top;
let fixedHeaderBlockHeight = fixedHeaderBlock.clientHeight;
...
function fixedHeader() {
  fixedHeaderBlockTopDistance = fixedHeaderBlock.getBoundingClientRect().top;
  headerBlockDistance = headerBlock.getBoundingClientRect().top;
  headerTopHeight = headerTop.clientHeight;
  if (window.matchMedia('(max-width: 650px)').matches) {
    if (headerBlockDistance >= 0) {
      fixedHeaderBlock.classList.remove('_fixed');
    }
  } else {
    if (fixedHeaderBlockTopDistance < 0) {
      fixedHeaderBlock.classList.add('_fixed');
      headerBlock.style.marginTop = `${fixedHeaderBlockHeight}px`;

    } else if (headerBlockDistance > fixedHeaderBlockHeight - headerTopHeight) {
      fixedHeaderBlock.classList.remove('_fixed');
      headerBlock.style.removeProperty('margin-top');
    }
  }
...
window.matchMedia('(max-width: 650px)').addEventListener('change', () => {
  if (window.matchMedia('(max-width: 650px)').matches) {
    if (fixedHeaderBlock.classList.contains('_fixed')) {
      headerBlock.style.marginTop = '44px';
    }
  } else {
    if (!fixedHeaderBlock.classList.contains('_fixed')) {
      headerBlock.style.removeProperty('margin-top');
...
if (document.querySelector('.three-rows-dots')) {
  let threeRows = document.querySelectorAll('.three-rows-dots');

  for (let item of threeRows) {
    $clamp(item, {
      clamp: 3,
      useNativeClamp: true,
    });
...
//double slider
if (document.querySelector('.double-slider')) {
  let doubleSliderWrapper = document.querySelector('.double-slider');
  let doubleSlider = doubleSliderWrapper.querySelector('.double-slider__object');

  let valLower = doubleSliderWrapper.querySelector('.double-slider__val-lower');
  let valUpper = doubleSliderWrapper.querySelector('.double-slider__val-upper');

  let snapValues = [valLower, valUpper];
  ...
  doubleSlider.noUiSlider.on('update', function (values, handle) {
...
//pagination disabled
if (document.querySelectorAll('.page-pagination__link').length) {
  let paginationLinks = document.querySelectorAll('.page-pagination__link');
  
  paginationLinks.forEach((link) => {
    if (link.classList.contains('_disabled')) {
...
if (document.querySelector('.brends-filter')) {
  const brendsFilter = document.querySelector('.brends-filter');
  let btns = brendsFilter.querySelectorAll('.brends-filter__btn');
  let blockItems = brendsFilter.querySelectorAll('.brends-filter__blocks-item');

  btns.forEach((btn) => {
    btn.addEventListener('click', function () {
      if (this.getAttribute('data-letter') === 'all') {
        for (let i = 0; i < btns.length; i++) {
          btns[i].classList.remove('_active');
        }  
        ...
        for (let i = 0; i < blockItems.length; i++) {
          blockItems[i].classList.add('_active');
          blockItems[i].classList.add('_add-line');
        }
      } else {
        for (let i = 0; i < btns.length; i++) {
          btns[i].classList.remove('_active');
        ...
        this.classList.add('_active');
        for (let i = 0; i < blockItems.length; i++) {
          blockItems[i].classList.remove('_add-line');
          if (this.getAttribute('data-letter') === blockItems[i].getAttribute('data-letter')) {
            blockItems[i].classList.add('_active');
          } else {
            blockItems[i].classList.remove('_active');
          }
        }
      }
...
if (document.querySelector('.product-catalog__block')) {
  const catalogBlock = document.querySelector('.product-catalog__block');
  const btns = catalogBlock.querySelectorAll('.product-catalog__btn');
  const blocks = catalogBlock.querySelectorAll('.product-catalog__item-block');
  const items = catalogBlock.querySelectorAll('.product-catalog__item');

  //start-settings
  let btnsDataName = btns[0].getAttribute('data-name');
  blockManagement(btnsDataName);
  ...
  btns.forEach((btn) => {
    btn.addEventListener('click', function () {
      if (!this.classList.contains('_active')) {
        ...
        blockManagement(btnName);
      }
    })
  })

function blockManagement(attrName) {
  for (let i = 0; i < blocks.length; i++) {
    let blockName = blocks[i].getAttribute('data-category');
    blockName = blockName.split(',');
    if (blockName.includes(attrName)) {
  ...
  items.forEach((item) => {
    let isActive = true;
    let itemBlocks = item.querySelectorAll('.product-catalog__item-block');
    for (let i = 0; i < itemBlocks.length; i++) {
      if (itemBlocks[i].classList.contains('_active')) {
        isActive = false;
      }
    }
    if (isActive) {
...
//category-form reset btn

if (document.querySelector('.category-filter__reset')) {
  const resetBtn = document.querySelector('.category-filter__reset');
  const selectInput = document.querySelector('.category-filter__select');
  const selectInputBtn = selectInput.querySelector('.custom-select__button');
  const selectInputBox = selectInput.querySelector('.custom-select__input');
  const selectInputOption = selectInput.querySelectorAll('.custom-select__option');

  resetBtn.addEventListener('click', function () {
    for (let i = 0; i < selectInputOption.length; i++) {
      if (selectInputOption[i].hasAttribute('active')) {
        selectInputBtn.textContent = selectInputOption[i].textContent;
        selectInputBox.value = selectInputOption[i].getAttribute('data-value');
      }
    }
  })
}

//category form mobile rebuild

if (document.querySelector('.category-filter')) {
  const categoryFilter = document.querySelector('.category-filter');
  const categoryFilterPopup = document.querySelector('.filter-popup')
  const categoryFilterOpenBtn = document.querySelector('.category-filter__open-btn');
  const categoryFilterPopupCloseBtn = document.querySelector('.category-filter-popup .popup__close');

  function maxWidth1100() {
    if (window.matchMedia('(max-width: 1100px)').matches) {
      categoryFilterPopup.prepend(categoryFilter);
    } else {
      categoryFilterOpenBtn.after(categoryFilter);
      categoryFilterPopupCloseBtn.dispatchEvent(new Event("click"));
    }
  };
...
function contactsMap(){
  ...
  var latlng = new google.maps.LatLng(55.7217501,37.6529002);
  var options = {
    scrollwheel: false,
    zoom: 18,
    panControl:false,
